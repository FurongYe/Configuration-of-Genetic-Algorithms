...
from bayes_optim.misc import handle_box_constraint
from bayes_optim.base import Solution
from bayes_optim.SearchSpace import ContinuousSpace, OrdinalSpace, NominalSpace
...
def dynamic_penalty(X, t, equality=None, inequality=None, C=0.5, alpha=1, beta=2,
    ...
    if equality is not None:
        v = np.atleast_1d(list(map(equality, X))).reshape(N, -1)
        v[np.abs(v) <= epsilon] = 0
        p += np.sum(np.abs(v), axis=1)

    if inequality is not None:
        v = np.atleast_1d(list(map(inequality, X))).reshape(N, -1)
        v[v <= 0] = 0
